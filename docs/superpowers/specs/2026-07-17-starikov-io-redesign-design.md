...
Tokens: `--dur-1..4: 100/150/200/300ms`, `--ease-out: cubic-bezier(0.16,1,0.3,1)`. Laws: transform/opacity only (color transitions exist solely in theme swap), user-initiated ≤300ms, interruptible, `prefers-reduced-motion` disables all but focus. Specifics: sidebar active bar slides via FLIP 180ms; page transitions fade 100ms out / 180ms in +4px rise with sidebar `transition:persist`; list entrance staggers first 10 rows only, first visit only; ⌘K panel scale 0.98→1 160ms, **filtering results: zero animation**; theme switch = 120ms linear tinted crossfade (no view-transition screenshots); copy ✓ spring; hero cursor blinks twice then holds.

...
**Amendment (2026-07-26, v1.1 polish Task 8):** two Task 3 refinements to the home hero statement, neither reflected above. (1) **Copy is governed by an editorial rule enforced at the source**, not just written once and left alone: `src/pages/index.astro`'s `STATEMENT` constant must stay one sentence, ≤90 characters, rendering in ≤4 lines at `.hero__statement`'s 24ch measure — a punchy answer to the mono `whoami` prompt above it, not a paragraph. Name and role were split out into their own line (`.hero__id`: "Illya Starikov — software engineer") so the statement itself never repeats them; the current copy ("The workshop: ten years of dotfiles, tools, and essays — documented like it matters.") measures 84 characters. (2) **The "44 (home statement only)" scale entry above is a ceiling, not a fixed size**: `--fs-statement` (`src/styles/tokens.css`) is a fluid `clamp(30px, 18px + 2.2vw, 44px)`, floored at 30px so the sentence stays legible without multi-line overflow at the 390px first viewport and reaching the full 44px only past roughly 1180px wide.
...
**Amendment (2026-07-26, v1.1 polish Task 4/8):** the scale above predates a concrete breakpoint scale — Task 4 introduced one, now normative and documented in full (with the reasoning for keeping two independent scales) in `src/styles/tokens.css`'s own comment block. **Viewport** `{640, 1024, 1280}`, declared with `@media`, for chrome-structural layout that depends on the actual window and can't be scoped to a container (the sidebar/rail grid, the mobile topbar vs. desktop sidebar). **Container** `{480, 560, 640, 720}`, declared with `@container content (...)` against the named container `Shell.astro`'s `.content-col` establishes (Task 3), for content-driven layout that should react to the space the reading column actually has — narrower with the sidebar/rail present (≥1024px) than without. Nothing outside these eight numbers should appear in a new media/container query without a documented, specific reason.
...
**Amendment (2026-07-26, v1.1 polish Task 8):** the line above is now an enforced CI gate, not just an aspiration -- `scripts/validate-dist.mjs` gzips (`node:zlib`'s `gzipSync`, level 9, matching the `gzip -9` every number here was measured with) every `dist/**/*.html` page against a per-page budget table (`PAGE_BUDGETS_KB`) and fails the build on any regression. The allowlist covers the pages carrying the award submission (§10): home, `/writing`, `/colophon`, `/academia`, `/projects/dotfiles`, `/changelog`, `/bin/pocketcasts-reset`; every other page (today: `/about`, `/404`, `/bin`, `/projects`, and the five non-`dotfiles` project overlays) shares one default budget sized off the heaviest of them. Methodology: measured on a clean build, rounded up to the next 0.5KB, plus one further 0.5KB step of headroom. gzip-9 is a stable local proxy for network cost, not the real one -- GitHub Pages serves through Fastly, which typically negotiates Brotli to modern browsers (meaningfully smaller than gzip-9 for HTML), so real production bytes are expected to land below every number here and the gate errs conservative, never optimistic --

| Page | Measured | Budget |
|---|---|---|
| `/` | 8.94KB gz | 9.5KB |
| `/writing` | 12.68KB gz | 13.5KB (see below) |
| `/colophon` | 13.52KB gz | 14.5KB |
| `/academia` | 13.87KB gz | 14.5KB |
| `/projects/dotfiles` | 11.98KB gz | 12.5KB |
| `/changelog` | 8.42KB gz | 9.0KB |
| `/bin/pocketcasts-reset` | 9.26KB gz | 10.0KB |
| everything else (default; worst case today: `/projects/mcp-servers` at 9.34KB gz) | 9.34KB gz | 10.0KB |

**`/writing` no longer fits the 12.5KB pin above -- found by this same measurement pass, not silently patched over.** Task 2's 11,887 B gz was accurate when taken; today's clean build measures 12,984 B gz (12.68KB) via this file's own `gzipSync` (the `gzip` CLI reads a few dozen bytes lower at the same level -- immaterial to the conclusion, but the gate checks itself against its own runtime, not a shell command run once by hand). The growth is real, reviewed content, not regression: Task 6 added a freshness-disclosure footer (`.writing__provenance`, this page's own honesty disclosure about which Ghost fetch actually served the build) and three right-rail facts (Essays/Tags/Feed) that previously didn't exist on this page at all. Re-pinning the gate at a number the very next build would already fail is a false alarm from minute one, not a regression gate -- so `/writing` gets the same measure-round-up-plus-headroom treatment as every other row above, honestly re-based at **13.5KB gz**. It remains the one budget in this table expected to need re-tuning again on its own over time, unrelated to any regression: it is the only page here rendering a full, ever-growing collection (currently 94 essays) rather than a fixed curated set, and each new essay is genuinely unique content that a shared-chrome-heavy page's gzip ratio can't fully absorb (Task 2's own finding, still true).

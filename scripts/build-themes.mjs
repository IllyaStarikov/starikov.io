...
import { loadThemeSource, loadAllFamilies } from './lib/theme-source.mjs';
...
// The full (uncurated) gallery data for /colophon: every family/variant on disk,
// swatches only, no CSS. See scripts/lib/theme-source.mjs's loadAllFamilies.
const OUT_ALL_JSON = join(OUT_ROOT, 'src/data/generated/themes-all.json');
...
function readSourceSha(dir) {
  // Only read a sha when `dir` is its OWN git checkout. CI's actions/checkout
  // populates .sources/dotfiles WITH a .git dir, so the palette's true source
  // commit is available. A local `sync-sources.sh` copy is an rsync WITHOUT
  // .git -- and an unguarded `git -C dir rev-parse HEAD` would then WALK UP into
  // THIS site's repo and return the SITE's HEAD, mislabeling it as the dotfiles
  // source sha (a false provenance line). Guarding on a real .git makes a local
  // build correctly report null, which /colophon renders as "dotfiles@snapshot".
...
function main() {
  ...
  // ---- themes-all.json: the whole engine, swatches only (the /colophon
  //      gallery). Read from the SAME source the curated build succeeded from,
  //      so gallery and picker never disagree about what shipped. ------------
  const allResult = loadAllFamilies(usedDir, manifest, { onWarn: warn });
  const allVariantCount = allResult.families.reduce((n, f) => n + f.variants.length, 0);
  if (allResult.omitted.length > 0) {
    warn(
      `themes-all: ${allResult.families.length} of ${Object.keys(manifest.families).length} ` +
        `families had colors.json in "${usedDir}"; gallery degraded to those ` +
        `(omitted: ${allResult.omitted.join(', ')})`,
    );
  }
  const themesAllJson = {
    families: allResult.families,
    meta: {
      source: usedKind,
      generatedAt: themesJson.meta.generatedAt,
      familyCount: allResult.families.length,
  ...
  mkdirSync(dirname(OUT_ALL_JSON), { recursive: true });
  ...
  writeFileSync(OUT_ALL_JSON, JSON.stringify(themesAllJson, null, 2) + '\n');

  const cssKb = (Buffer.byteLength(cssOut, 'utf8') / 1024).toFixed(1);
  console.log(
    `themes: ${families.length} families, ${emitted.length} variants, css ${cssKb} KB; ` +
      `gallery ${allResult.families.length} families / ${allVariantCount} variants`,
  );
}
